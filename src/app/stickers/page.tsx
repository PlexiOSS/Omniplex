...
import { Layers, Sticker as StickerIcon } from "lucide-react";
...
export default function StickersPage() {
  ...
  const { data, isLoading, error } = useSWR(`stickers/all/${page}`, () =>
    stickers.getAll(page),
  );
    ...
    <Container className="py-10">
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-lg text-sm text-zinc-500 dark:text-zinc-400">
            Every sticker from every Sticker Pack, browsable on its own. Click
            one to see who made it, which pack it's from, and download it
            individually.
          </p>
        </div>
        <Link
          href="/packs?pack_type=sticker"
      ...
      {error ? (
        <div className="flex flex-col items-center justify-center py-24 text-zinc-500 dark:text-zinc-400">
          <p className="text-sm">Failed to load stickers.</p>
        </div>
      ) : isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center text-zinc-500 dark:text-zinc-400">
          ...
          />
          <p className="text-sm">No stickers have been uploaded yet.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(96px,1fr))] gap-3">
            {results.map((sticker) => (
              <Link
                ...
                title={`${sticker.name} from ${sticker.pack_name}`}
