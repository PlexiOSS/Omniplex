...
interface TeamVoteButtonProps {
  teamId: string;
  currentVotes: number;
}

export function TeamVoteButton({ teamId, currentVotes }: TeamVoteButtonProps) {
  const router = useRouter();
  const { session, isAuthenticated } = useAuth();
  const { vote, loading, error } = useVote("team", teamId);
  ...
  // Teams allow multiple votes over time, but only one vote (in either
  ...
  const handleVote = async (upvote: boolean) => {
    ...
    // Overview tab was fetched server-side before the vote -- refresh so it
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <Modal
        ...
        title="Downvote this team?"
      >
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Downvotes can't be removed or changed once submitted, and you can only
          vote once every {voteCooldownHours(false)} hours. Are you sure?
        </p>
        <div className="mt-5 flex justify-end gap-2">
          <Button
            variant="secondary"
            onClick={() => setConfirmingDownvote(false)}
          >
            Cancel
          </Button>
